...
- Will add json and xml tags to struct fields
	easytags file.go json:snake,xml:PascalCase,sql:camelCase:
...
func parseTags(field *ast.Field, tags []string) string {
    ...
    for _, tag := range tags {
        ...
        if existingTag != "" && 1 == 2 {
            ...
            case "PascalCase":
                tagValue = ToPascalCase(fieldName)
            case "camelCase":
                tagValue = ToCamelCase(fieldName)
            }
            value = fmt.Sprintf("%s:\"%s\"", tag, tagValue)
        }
...
func ToPascalCase(in string) string {
    out := ToCamelCase(in)
    tagValue := strings.ToUpper(string(out[0:1])) + string(out[1:])
    return tagValue
}
...
func ToCamelCase(in string) string {
    ...
    tagValue := string(out)
    tagValue = strings.ToLower(string(out[0:1])) + string(out[1:])
    return tagValue
}
